...
class SignatureDetector:
    """
    Detect files by matching their hashes
    ...
    def check_hashes(
        ...
        sha256_matches = signatures.get(
            "sha256",
            {}
        )

        md5_matches = signatures.get(
            "md5",
            {}
        )

        sha256_match = sha256_matches.get(
            sha256.lower()
        )

        md5_match = md5_matches.get(
            md5.lower()
        )
