...
class UserCreate(UserBase):
    """Schema for user registration request payload (Role is assigned automatically by server)."""
    password: str = Field(
        ...,
        min_length=8,
        max_length=128,
...
class UserUpdate(BaseModel):
    """Schema for updating user profile information (Role cannot be modified by user)."""
...
class UserResponse(UserBase):
    ...
    role: str = Field(default="analyst", description="User authorization role (assigned by server)")
